...
const ExpensesForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className='w-full flex flex-col items-center rounded-xl border-2 border-gray-700 backdrop-blur-lg	justify-evenly shadow-2xl'>
      <Input label={"הוצאה"} type={'text'} id={"expenses"}  />
      <Input label={"date"} type={'date'} id={"date"} />
      <Input label={"סכום"} type={'number'} id={"sum"} />
      <SelectInput label={"סוג הוצאה"} type={'text'} id={"expenses-type"} data={typeList} />
      <SelectInput label={"של מי"} id={"whom"} data={whosList} />
